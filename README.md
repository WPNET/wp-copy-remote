...
- 🧹 **Optional SQL Sanitizing**: Use `-f` to strip privileged SQL statements before import
...
-e, --exclude "LIST"         Space-delimited paths to exclude
-p, --install-plugins "LIST" Plugins to install after sync
-r, --remote-cmds "CMD"      Commands to run on destination after sync
...
# Unattended with exclusions
./.local/bin/wp-pull-remote -u -e "uploads cache .git"

# Install plugins on destination after sync
./.local/bin/wp-push-remote --install-plugins "woocommerce contact-form-7"

# Enable SQL filtering (for restrictive MySQL permissions)
./.local/bin/wp-push-remote -f

# Run commands on destination after sync
...
6. Cleanup temp files on remote and local

## Configuration

Run `--config` once to save settings. Fields prompted:
